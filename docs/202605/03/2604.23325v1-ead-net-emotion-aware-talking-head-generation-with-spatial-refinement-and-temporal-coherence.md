...
title_zh: EAD-Net：结合空间细化与时间相干性的情感感知说话人头像生成
authors: "Yahui Li, Yinfeng Yu, Liejun Wang, Shengjie Shen"
date: 2026-04-25
pdf: "https://arxiv.org/pdf/2604.23325v1"
tags: ["query:3d-motion-generation"]
score: 6.5
evidence: 基于扩散模型的网络，用于生成富有表现力的肖像视频和面部表情
tldr: 本研究提出EAD-Net，一种基于扩散模型的情感感知说话人视频生成网络。针对现有方法情感语义不足、长视频全局运动感知差及唇形同步退化等问题，该模型引入SyncNet监督与时间表示对齐技术，并利用大语言模型提取高层情感语义。通过时空定向注意力和时间帧图推理模块，显著提升了视频的连贯性与表现力。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2604.23325v1/fig-001.webp\", \"caption\": \"\", \"page\": 3, \"index\": 1, \"width\": 1788, \"height\": 256}]"
motivation: 现有情感说话人生成方法存在情感语义缺失、长视频运动建模困难以及引入高层语义导致的唇形同步退化问题。
method: 提出结合SyncNet监督、时空定向注意力机制（STDA）和时间帧图推理模块（TFRM）的扩散模型，并利用LLM提取文本描述作为情感引导。
result: 在HDTF和MEAD数据集上的实验表明，该方法在唇形同步准确度、时间一致性和情感表达准确性方面均优于现有技术。
conclusion: EAD-Net通过多模态融合优化与复杂的时空依赖建模，有效解决了情感视频生成中的质量与一致性平衡挑战。
---

## 摘要
情感说话人头像视频生成旨在生成具有准确口型同步和情感面部表情的富有表现力的肖像视频。目前的方法依赖于简单的情感标签，导致语义信息不足。虽然引入高层语义可以增强表现力，但容易导致口型同步性能下降。此外，主流生成方法在长视频中难以平衡计算效率与全局运动感知，且存在时间相干性差的问题。因此，我们提出了一种基于情感感知扩散模型的网络，称为 EAD-Net。我们引入了 SyncNet 监督和时间表示对齐（TREPA），以减轻多模态融合导致的口型同步退化。为了建模长视频序列中复杂的时空依赖关系，我们提出了一种时空定向注意力（STDA）机制，通过条带注意力捕捉全局运动模式。此外，我们设计了一个时间帧图推理模块（TFRM），通过图结构学习显式建模视频帧之间的时间相干性。为了增强情感语义控制，利用大语言模型从真实视频中提取文本描述，作为高层语义引导。在 HDTF 和 MEAD 数据集上的实验表明，我们的方法在口型同步准确性、时间一致性和情感准确性方面优于现有方法。
