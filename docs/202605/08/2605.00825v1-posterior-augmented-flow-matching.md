...
tldr: 本研究针对流匹配（Flow Matching）在高维图像生成中因单样本监督导致的训练信号稀疏和流崩溃问题，提出了后验增强流匹配（PAFM）。该方法通过重要性采样构建多个候选目标的混合分布，将单目标监督扩展为对有效目标后验的期望，从而显著降低梯度方差并提升泛化能力。实验表明，PAFM在多个基准测试中均优于传统流匹配方法。
source: arxiv
...
motivation: 传统流匹配在处理高维数据时因监督信号过于稀疏且方差大，容易导致模型陷入流崩溃并丧失泛化能力。
method: 提出PAFM框架，利用重要性采样将单目标监督替换为对给定中间状态下所有可能目标的后验期望。
result: 在ImageNet和CC12M等基准上，PAFM在不同模型规模和架构下均实现了高达3.4的FID提升，且计算开销极小。
conclusion: PAFM通过聚合多条可能的延续轨迹信息，提供了一个无偏且低方差的流匹配估计器，有效解决了高维生成任务中的训练不稳定性。
---

## 摘要
流匹配 (Flow Matching, FM) 训练一个随时间变化的向量场，将样本从简单的先验分布传输到复杂的数据分布。然而，对于高维图像，每个训练样本仅监督单一的轨迹和中间点，导致训练信号极其稀疏且具有高方差。这种约束不足的监督可能导致流崩溃 (flow collapse)，即学习到的动力学记住了特定的源-目标配对，将多样的输入映射到过于相似的输出，从而无法泛化。我们引入了后验增强流匹配 (Posterior-Augmented Flow Matching, PAFM)，这是 FM 的一种具有理论基础的泛化形式，它将单目标监督替换为给定中间状态和条件下有效目标补全的近似后验期望。PAFM 将这一难以处理的后验分解为：(i) 假设终点下中间状态的似然，以及 (ii) 该终点在条件下的先验概率，并使用重要性采样方案构建多个候选目标的混合。我们证明了 PAFM 产生了原始 FM 目标的无偏估计量，同时通过聚合每个中间状态的许多可能延续轨迹的信息，显著降低了训练过程中的梯度方差。最后，我们展示了 PAFM 在不同模型规模 (SiT-B/2 和 SiT-XL/2)、不同架构 (SiT 和 MMDiT) 以及类别和文本条件基准 (ImageNet 和 CC12M) 上，相比 FM 在 FID50K 指标上提升了高达 3.4，且计算开销的增加微乎其微。代码：https://github.com/gstoica27/PAFM.git。
