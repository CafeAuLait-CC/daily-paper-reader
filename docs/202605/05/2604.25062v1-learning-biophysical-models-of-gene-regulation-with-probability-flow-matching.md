...
score: 6.0
evidence: 用于学习随机过程的概率流匹配方法
tldr: 细胞分化由复杂的基因调控网络驱动，但现有单细胞分析方法往往缺乏物理可解释性且泛化能力差。本文提出概率流匹配（PFM）框架，能从时间序列单细胞数据中学习符合生物物理规律的随机过程。通过在造血数据集上的应用，PFM证明了其在捕捉谱系转换、命运决定及基因扰动响应方面的优越性，并能同时推断细胞增殖与死亡动态，为单细胞组学提供了可扩展的机械建模工具。
source: arxiv
...
motivation: 现有的单细胞基因调控动态推断方法缺乏机械可解释性，且难以泛化到未见过的实验条件。
method: 提出概率流匹配（PFM）框架，利用时间分辨的单细胞测量数据直接学习具有生物物理一致性的随机过程。
result: PFM在造血数据集中准确捕捉了谱系转换和扰动响应，并能处理不平衡群体以推断细胞增殖与死亡。
conclusion: PFM是一个灵活且可扩展的框架，成功实现了机械建模与单细胞组学数据的高效整合。
---

## 摘要
细胞分化受基因调控网络支配，这些高维随机生化系统决定了转录景观并介导细胞对信号和扰动的响应。尽管单细胞RNA测序提供了转录组的定量快照，但目前推断基因调控动力学的方法往往缺乏机械解释性，且无法泛化到未见过的条件。在此，我们介绍了概率流匹配（PFM），这是一个可扩展的框架，用于直接从时间分辨的单细胞测量中学习生物物理一致的随机过程。通过将PFM应用于三个造血数据集，我们展示了具有相似插值准确度的模型可能编码截然不同的动力学，只有生物物理一致的公式才能准确捕捉谱系转换、命运决定和基因扰动响应的机制。我们进一步证明PFM能够处理不平衡的群体，从而实现对细胞增殖和死亡动力学的同步推断。总之，这些结果确立了PFM作为一个灵活、可扩展的框架，用于将机械建模与单细胞组学相结合。
