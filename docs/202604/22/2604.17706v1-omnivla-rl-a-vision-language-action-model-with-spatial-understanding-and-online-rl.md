...
title_zh: OmniVLA-RL：一种具备空间理解与在线强化学习的视觉-语言-动作模型
...
tldr: 本研究针对现有视觉-语言-动作模型在空间感知、多模态融合和强化学习稳定性方面的不足，提出了OmniVLA-RL模型。该模型采用Mix-of-Transformers架构协同整合推理、空间和动作专家，并引入Flow-GSPO方法将流匹配重构为随机微分方程过程，结合分组分段策略优化提升动作精度和训练鲁棒性。在LIBERO系列基准上的评估表明，该模型性能显著优于现有方法。
source: arxiv
...
motivation: 现有视觉-语言-动作模型存在空间感知不精确、多模态融合欠佳和强化学习不稳定等问题，需要改进。
method: 提出OmniVLA-RL架构，采用Mix-of-Transformers集成专家模块，并引入Flow-GSPO方法增强动作精度和训练鲁棒性。
result: 在LIBERO和LIBERO-Plus基准测试中显著优于现有最先进方法。
conclusion: 该模型有效克服了当前VLA模型在空间感知、多模态融合和强化学习稳定性方面的根本局限。
---

## 摘要
视觉-语言-动作（VLA）模型体现了具身AI的范式转变，但现有框架常受限于空间感知不精确、多模态融合欠佳以及强化学习不稳定等问题。为弥补这些不足，我们提出了OmniVLA-RL，这是一种新颖的架构，利用混合Transformer（MoT）设计协同整合推理、空间和动作专家。此外，我们引入了Flow-GSPO，它将流匹配重新表述为随机微分方程（SDE）过程，并与分组分段策略优化（GSPO）相结合，以提升动作精度和训练鲁棒性。在LIBERO和LIBERO-Plus基准上的广泛评估表明，OmniVLA-RL显著优于现有最先进方法，有效克服了当前VLA模型的基本局限性。
