...
score: 7.0
evidence: 用于预测复杂运动模式的扩散Transformer
tldr: 本研究针对视觉系统缺乏通用运动表示的问题，提出将密集点轨迹作为行为的视觉标记。该方法将运动与外观解耦，能有效处理自然场景中动物等非刚性物体的复杂行为。通过构建扩散变换器模型并结合大规模动物视频数据集，实现了对遮挡敏感且连贯的运动预测。实验证明，该方法具有极强的跨类别泛化能力，在预测精度和数据效率上均优于现有技术，为野外环境下的预测性视觉智能提供了新范式。
source: arxiv
...
motivation: 现有的视觉系统缺乏一种通用的、能跨类别泛化且能处理非刚性物体复杂行为的运动表示方法。
method: 提出将密集点轨迹作为视觉标记，并利用扩散变换器对无序轨迹集进行建模，以显式处理遮挡并预测连贯运动。
result: 在300小时的动物视频数据集上，该方法实现了类别无关的高效预测，并在处理稀有物种和复杂形态时表现优于基准模型。
conclusion: 密集点轨迹作为一种中层结构化表示，为实现自然场景下通用的预测性视觉智能奠定了坚实基础。
---

## 摘要
视觉智能需要预测智能体的未来行为，然而视觉系统目前仍缺乏一种通用的运动和行为表示。我们提出将密集点轨迹作为行为的视觉标记，这是一种结构化的中层表示，能够将运动与外观解耦，并可泛化至各种非刚性智能体（如自然场景中的动物）。基于这一抽象表示，我们设计了一个扩散 Transformer，用于对无序轨迹集进行建模并显式推理遮挡，从而实现对复杂运动模式的连贯预测。为了进行大规模评估，我们整理了 300 小时的非受限动物视频，并应用了鲁棒的分镜检测和相机运动补偿技术。实验表明，预测轨迹标记实现了类别无关且数据高效的预测，性能优于最先进的基准模型，并能泛化至稀有物种和形态，为自然场景下的预测性视觉智能奠定了基础。
