...
tldr: 本研究提出了一种基于条件流匹配（CFM）的框架，用于解决受物理约束的贝叶斯反问题。该方法通过神经网络学习概率流常微分方程的向量场，将源分布直接映射到给定观测值的后验分布，无需显式评估先验或似然。研究深入分析了有限数据下的过拟合现象（如方差塌陷和选择性记忆），并证明了早停策略的有效性。实验表明，该方法能高效捕捉复杂的多峰后验分布，适用于高维非线性物理模型。
selection_source: fresh_fetch
motivation: 旨在解决传统贝叶斯反问题中对先验和似然显式评估的依赖，以及处理高维非线性物理模型时的计算挑战。
method: 采用条件流匹配技术，通过训练神经网络学习速度场，实现从简单源分布到复杂后验分布的确定性传输。
result: 实验证明该方法能准确捕捉多峰后验分布，并揭示了过训练导致的退化行为及其通过早停法的缓解方案。
conclusion: 条件流匹配为物理约束反问题提供了一种高效、灵活且无需似然函数的推断框架，具有良好的泛化能力和计算效率。
---
