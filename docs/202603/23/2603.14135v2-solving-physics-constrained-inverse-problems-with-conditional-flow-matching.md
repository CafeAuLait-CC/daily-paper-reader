...
tldr: 本研究提出了一种基于条件流匹配（CFM）的框架，用于解决受物理约束的贝叶斯反问题。该方法通过学习概率流常微分方程的向量场，将源分布直接映射到给定观测值的后验分布，无需显式评估先验或似然。研究深入分析了有限训练数据下的过拟合现象（如方差塌陷和选择性记忆），并验证了早停策略的有效性。实验表明，该方法能高效捕捉复杂的多峰后验分布，适用于高维非线性物理模型。
selection_source: fresh_fetch
motivation: 旨在解决传统贝叶斯反问题中显式似然计算困难以及高维非线性物理模型难以处理的挑战。
method: 提出一种条件流匹配框架，通过训练神经网络学习速度场，将简单源分布变换为复杂的条件后验分布。
result: 实验证明该方法能准确捕捉多峰后验分布，并揭示了有限数据下过拟合导致的方差塌陷和选择性记忆问题。
conclusion: 条件流匹配为物理约束反问题提供了一种高效且灵活的黑盒求解方案，通过适当的正则化可有效处理复杂推断任务。
---
