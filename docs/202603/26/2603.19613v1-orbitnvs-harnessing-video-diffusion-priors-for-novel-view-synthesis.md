...
tldr: OrbitNVS将新视角合成（NVS）重新定义为轨道视频生成任务，旨在解决单视角输入下生成未观测区域时的几何与外观一致性难题。该方法通过在预训练视频扩散模型中引入相机适配器实现精确控制，并设计法向图生成分支以增强几何约束，同时利用像素级监督提升图像清晰度。实验表明，OrbitNVS在GSO和OmniObject3D基准测试中显著优于现有方法，尤其在单视角设置下表现出色。
selection_source: fresh_fetch
motivation: 现有NVS方法在处理单视角输入时，难以在生成未观测区域时维持良好的几何一致性和外观细节。
method: 将NVS转化为轨道视频生成任务，通过相机适配器、法向图引导分支和像素级监督对预训练视频扩散模型进行改进。
result: 在GSO和OmniObject3D基准测试中，OrbitNVS在单视角设置下的PSNR分别提升了2.9 dB和2.4 dB，性能大幅领先。
conclusion: 通过结合视频扩散模型的强大先验与几何约束设计，OrbitNVS有效提升了新视角合成的质量与一致性。
---
