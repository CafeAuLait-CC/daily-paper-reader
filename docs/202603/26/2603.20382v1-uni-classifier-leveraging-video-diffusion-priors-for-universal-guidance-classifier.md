...
## 摘要
在实际的 AI 工作流中，复杂任务通常涉及串联多个生成模型，例如在 2D 图像生成器之后使用视频或 3D 生成模型。然而，上游模型的输出与下游模型预期的输入之间的分布不匹配经常会导致整体生成质量下降。为了解决这一问题，我们提出了 Uni-Classifier (Uni-C)，这是一个简单且有效的即插即用模块，它利用视频扩散先验来引导前序模型的去噪过程，从而使其输出与下游需求保持一致。Uni-C 也可以独立应用，以增强单个生成模型的输出质量。在视频和 3D 生成任务上的广泛实验表明，Uni-C 在基于工作流和独立设置下都能持续提高生成质量，突显了其多功能性和强大的泛化能力。
