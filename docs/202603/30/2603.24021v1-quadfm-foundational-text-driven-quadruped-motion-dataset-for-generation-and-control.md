...
tldr: 针对四足机器人缺乏多样化动作及语言语义关联的问题，本文提出了首个大规模、高保真文本驱动动作数据集 QuadFM，包含 1.1 万余个涵盖运动、交互及情感表达的动作片段及 3.5 万条三层级文本标注。同时提出 Gen2Control RL 框架，实现了文本到动作生成与控制器的联合训练，在边缘设备上实现了低延迟的实时动作合成与稳健控制。
source: arxiv
...
motivation: 现有的四足机器人动作数据集规模有限且缺乏丰富的语言语义关联，难以支持直观的人机交互和多样化行为生成。
method: "引入包含 11,784 个动作片段的大规模数据集 QuadFM，并提出 Gen2Control RL 框架，通过联合训练生成器与控制器实现端到端推理。"
result: 在真实机器人和仿真环境中实现了低于 500 毫秒的实时动作合成，展现了高度多样化且物理稳健的动作表现。
conclusion: QuadFM 为四足机器人提供了基础的运动资源和高效的控制框架，显著提升了机器人理解自然语言指令并执行复杂动作的能力。
---

## 摘要
尽管四足机器人技术取得了显著进展，但在基础运动资源方面仍存在关键空白，即缺乏能够全面整合多样化步态、情感表达行为以及丰富语言语义的资源，而这些对于实现敏捷、直观的人机交互至关重要。目前的四足机器人运动数据集仅限于少数动捕原语（如行走、小跑、坐下），缺乏具有丰富语言关联的多样化行为。为了填补这一空白，我们推出了 Quadruped Foundational Motion (QuadFM)，这是首个专为文本到运动生成和通用运动控制设计的大规模、超高保真数据集。QuadFM 包含 11,784 个精心挑选的运动片段，涵盖了步态、交互和情感表达行为（如跳舞、伸展、排尿），每个片段都带有三层标注——细粒度动作标签、交互场景和自然语言指令——总计 35,352 条描述，以支持基于语言条件的理解和指令执行。我们进一步提出了 Gen2Control RL，这是一个统一的框架，可联合训练通用运动控制器和文本到运动生成器，从而在边缘硬件上实现高效的端到端推理。在配备 NVIDIA Orin 的真实四足机器人上，我们的系统实现了实时运动合成（延迟 <500 ms）。仿真和真实世界的结果显示，该系统在保持稳健物理交互的同时，能够产生逼真且多样化的运动。该数据集将在 https://github.com/GaoLii/QuadFM 发布。
