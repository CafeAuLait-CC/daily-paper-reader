...
title_zh: 扩散模型中准确梯度估计的时间反向倒向随机微分方程
...
score: 6.0
evidence: 扩散模型中的梯度估计
tldr: 本研究针对扩散模型微调中的梯度估计问题，指出传统伴随匹配方法因伴随过程未适应前向过滤而导致梯度估计不稳定且方差较高。为此，作者从倒向随机微分方程（BSDE）视角出发，提出了一种基于时间反向 BSDE 的新型梯度估计器。该方法通过构建适应底层过滤的伴随过程，显著提升了梯度估计的稳定性并降低了方差。在玩具模型上的实验验证了该方法在微调扩散模型时的优越性能。
selection_source: fresh_fetch
motivation: 现有的扩散模型微调方法由于伴随过程不适应前向过滤，导致梯度估计存在不稳定性及高方差问题。
method: 提出一种基于时间反向 BSDE 的梯度估计器，通过构建适应性伴随过程来优化随机控制问题。
result: 数值实验证明，该方法在微调扩散模型时提供了更稳定的梯度估计，并展现出优异的性能。
conclusion: 时间反向 BSDE 框架为扩散模型提供了更精确的梯度估计手段，有效提升了生成策略微调的稳定性。
---
