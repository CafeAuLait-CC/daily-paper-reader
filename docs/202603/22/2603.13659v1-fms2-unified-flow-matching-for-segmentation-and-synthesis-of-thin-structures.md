...
## 摘要
分割基础设施裂缝和解剖血管等细微结构是一项具有挑战性的任务，受限于拓扑敏感的几何形状、高昂的标注成本以及较差的跨域泛化能力。现有方法通常孤立地解决这些挑战。我们提出了 FMS$^2$，这是一个包含两个模块的流匹配框架。(1) SegFlow 是一个拥有 296 万参数的分割模型，构建在标准的编码器-解码器骨干网络之上，它将预测重构为连续的图像到掩码（image $\rightarrow$ mask）传输过程。它通过流匹配回归损失学习随时间索引的速度场，并利用常微分方程（ODE）积分输出掩码，而非仅监督最终状态的逻辑值（logits）。与经过调优的拓扑感知损失基准相比，这种轨迹级监督在无需辅助拓扑头、后处理或多项损失工程的情况下，提升了细微结构的连续性和清晰度。(2) SynFlow 是一个以掩码为条件的掩码到图像（mask $\rightarrow$ image）生成器，用于产生像素对齐的合成图像-掩码对。它在多个尺度上注入掩码几何信息，并通过边缘感知门控强调边界带，同时一个可控的掩码生成器扩展了稀疏度、宽度和分支机制。在五个裂缝和血管基准测试中，仅 SegFlow 就优于强大的 CNN、Transformer、Mamba 和生成式基准，将体积指标（平均交并比 mIoU）从 0.511 提升至 0.599（+17.2%），并将拓扑指标（Betti 匹配误差）从 82.145 降低至 51.524（-37.3%）。在有限标签训练时，使用 SynFlow 生成的样本对增强 SegFlow，仅需 25% 的真实标注即可恢复接近完整的性能，并使跨域 IoU 平均提高 0.11。与通过保持标签的变换来提升不变性的传统数据增强不同，SynFlow 提供了具有可控结构偏移（如稀疏度、宽度、分支）的像素对齐配对监督，这在领域偏移情况下尤为有效。
