...
score: 6.0
evidence: 扩散模型生成动力学
tldr: 扩散模型随机生成动力学常需额外估计score或重构速度，而现有速度模型多为确定性单场预测，难以直接支持随机反向过程。本文提出均值速度匹配MVM，构造高斯扰动过程，使恢复导向速度的条件期望直接成为反向SDE漂移，无需单独估计score，仅用单一学习场即可参数化随机反向过程。MVM还引入sqrt(t)缩放参数化，避免t→0时目标无界，并可由同一场导出概率流ODE，统一随机与确定性采样。实验在ImageNet 32×32与256×256取得有竞争力FID，且极低NFE时ODE更优、NFE充足时随机反向过程FID更低。
source: arxiv
...
motivation: 现有速度生成模型虽能单场学习传输，但标准形式确定，随机扩展常需额外score或速度到score重构。
method: MVM构造高斯扰动，使恢复导向速度(x0-xt)/t的条件期望直接作反向SDE漂移，单场参数化随机反向；sqrt(t)缩放避免t→0无界，并导出概率流ODE。
result: Transformer生成模型在ImageNet 32×32与256×256取得有竞争力FID；极低NFE下ODE更优，NFE充足时随机反向过程FID更低。
conclusion: MVM为随机反向动力学提供直接单场参数化，统一随机与确定性采样，并保持有竞争力的生成质量。
---

## 摘要
本文研究扩散模型中随机生成动力学的预测参数化。现有的基于速度的生成模型具有学习单一传输场的简洁性，但其标准形式是确定性的，而随机扩展通常需要额外的分数信息或中间的速度到分数重建。为了在直接支持随机逆向动力学的同时保留单场预测，本文提出均值速度匹配（Mean Velocity Matching, MVM）。MVM 构建了一个高斯扰动过程，其中面向恢复的速度 $(x_0-x_t)/t$ 的条件期望直接构成反向 SDE 的漂移项。因此，单个学习到的场就足以参数化随机反向过程，而无需单独估计或重建分数。由于对该速度的直接回归在 $t=0$ 附近会变得无界，MVM 进一步引入了 $\sqrt{t}$ 缩放的参数化，在保持反向动力学的同时产生有界的训练目标。同一个学习到的场还诱导出一个确定性的概率流 ODE，使得随机采样和确定性采样可以在统一框架下研究。基于 Transformer 的生成模型实验在 ImageNet $32\times32$ 上以 \MVMImageNetThirtyTwoNFE\ NFE 达到 $\MVMImageNetThirtyTwoFID$ 的 FID，并在 ImageNet $256\times256$ 上以 \MVMImageNetTwoFiftySixNFE\ NFE 达到 $\MVMImageNetTwoFiftySixFID$ 的 FID。受控的 SDE--ODE 比较进一步表明，在极低 NFE 下 ODE 表现更好，而当有足够多的函数评估时，随机反向过程获得更低的 FID。这些结果表明，MVM 为随机反向动力学提供了直接的单场参数化，同时保持了有竞争力的生成质量。
