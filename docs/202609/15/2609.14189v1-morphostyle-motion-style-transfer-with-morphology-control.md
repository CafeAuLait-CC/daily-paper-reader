...
tags: ["query:dmg"]
score: 6.0
evidence: 形体感知的人体动作风格迁移与生成
tldr: 现有动作风格迁移通常假设标准体型，而体型感知生成又缺乏显式风格控制，难以生成非标准体型的风格化动作。MorphoStyle 基于体型条件 FSQ-VAE，将风格迁移建模为模块化潜空间解耦。它用对比风格编码器、文本引导风格路由和保流形风格调制器，在内容特征中注入低秩风格偏移。实验表明其在形状控制与风格迁移上均优于基线，并支持定量形状控制。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2609.14189v1/fig-001.webp\", \"caption\": \"\", \"page\": 11, \"index\": 1, \"width\": 463, \"height\": 260}, {\"url\": \"assets/figures/arxiv/2609.14189v1/fig-002.webp\", \"caption\": \"\", \"page\": 13, \"index\": 2, \"width\": 433, \"height\": 420}, {\"url\": \"assets/figures/arxiv/2609.14189v1/fig-003.webp\", \"caption\": \"\", \"page\": 13, \"index\": 3, \"width\": 837, \"height\": 649}]"
motivation: 现有风格迁移假设标准体型，体型感知生成缺少风格控制，简单组合会泄漏内容并破坏形状一致运动学。
method: MorphoStyle 以体型条件 FSQ-VAE 为基础，通过对比风格编码器、文本引导风格路由和保流形低秩风格调制器解耦内容、风格与形态。
result: 在基准数据集上，MorphoStyle 在形状控制与动作风格迁移两方面均优于竞争基线，并提供定量形状控制。
conclusion: 该框架将风格与形态统一建模，使非标准体型也能获得内容解耦、形状一致的风格化动作，推进可控动作生成。
---

## 摘要
人体运动可以视为动作内容、风格和身体形态的组合。现有的运动风格迁移方法在假设标准体型的前提下，将参考风格迁移到内容运动上；而形状感知的运动生成器则使运动适应目标体型，但不具备显式的风格控制。这种运动风格与形状（形态）的分离，使得为非标准体型生成风格化运动变得困难；简单地将风格迁移模块与形状感知生成器组合，往往会泄漏来自风格参考的动作内容，并破坏与形状一致的运动学。我们提出 MorphoStyle，一个基于形状条件化的有限标量量化变分自编码器（FSQ-VAE）的形状感知运动风格迁移框架。其关键贡献在于将所需的风格迁移表述为模块化潜在解耦，包括：（i）对比式风格编码器，用于提取与内容解耦的风格嵌入；（ii）文本引导的风格路由机制，用于在文本-运动特征空间中定位风格相关关节；（iii）流形保持的风格调制器，以时间门控的低秩偏移形式将判别性风格嵌入注入内容特征。在基准数据集上的大量实验表明，MorphoStyle 在形状控制和运动风格迁移两方面均优于竞争基线，同时还能提供定量的形状控制。更多细节请参见项目网站：https://github.com/funkdub/MorphoStyle。
