...
tldr: "开放环境桌面机器人操作需要同时具备语义理解、精确几何位姿估计和高频动作生成能力。现有端到端VLA模型缺乏显式几何约束且训练成本高。为此提出OpenSPM框架，利用语义条件3D感知和卡尔曼滤波追踪6D位姿，从人类演示中提取可迁移的关键空间位姿作为持久记忆；推理时根据语言指令检索并迁移位姿，由轻量条件流匹配模型生成高频动作块，结合闭环残差校正抑制误差积累。在LIBERO-GOAL十项任务上达到85.6%成功率和1033.3 Hz等效控制频率，且推理算力需求低。该工作弥合了高层语义与底层物理执行间的鸿沟，为可靠高频操作提供了有效方案。"
source: arxiv
...
motivation: 现有端到端VLA模型缺乏显式几何约束且训练成本高，难以同时满足语义泛化和精确位姿需求。
method: 构建空间位姿记忆和流匹配动作生成模型，融合语义感知、SE(3)迁移与闭环残差校正，生成高频动作。
result: "在LIBERO-GOAL基准上达85.6%成功率和1033.3 Hz等效控制频率，推理算力低。"
conclusion: 结构化空间持久记忆与闭环残差校正对可靠高频操作至关重要，弥合了语义与物理执行的鸿沟。
---

## 摘要
开放环境下的桌面机器人操作要求系统具备语义理解、精确几何姿态估计和高频动作生成能力。端到端的视觉-语言-动作（VLA）模型擅长语义泛化，但往往缺乏细粒度任务所需的显式几何约束，且需要昂贵的训练成本。为弥合高层语义与低层物理执行之间的鸿沟，我们提出OpenSPM，一种开放环境空间持久记忆框架，由空间姿态记忆和流匹配动作生成模型组成。OpenSPM首先利用语义条件化的3D感知和卡尔曼滤波来跟踪连续的6D姿态，然后从人类演示中提取关键空间姿态，将其作为可迁移的、以物体为中心的空间持久记忆条目。在推理过程中，OpenSPM根据自然语言指令检索相关记忆条目，利用SE(3)变换将空间姿态迁移到新场景，并通过轻量级条件流匹配模型生成高频动作块。结合实时本体感知状态反馈和终端残差校正，系统有效抑制了轨迹误差累积。在十个LIBERO-GOAL任务上的评估显示，OpenSPM实现了85.6%的成功率和1033.3 Hz的等效控制频率，同时仅需极少的推理AI算力。大量消融实验表明，结构化的空间持久记忆和闭环残差校正在可靠的高频机器人操作中起着关键作用。
